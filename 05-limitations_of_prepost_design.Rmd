...
```{r trogchangedata,echo=FALSE,message=FALSE,warning=FALSE,fig.cap='TROG data from a study conducted by @bishop2006',fig.width=8,fig.height=4}
require(reshape2)
...
p <- ggplot(df2, aes(x=Group, y=TROG_Mean_scaled_score, fill=Occasion)) +
  ...
  geom_errorbar(aes(ymin=TROG_Mean_scaled_score-se, ymax=TROG_Mean_scaled_score+se), width=.2,position=position_dodge(.9))+ylab('TROG Mean scaled score') +xlab('Intervention Group')
