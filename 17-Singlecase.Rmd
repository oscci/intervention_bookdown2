...
Given the large number of items in each set, it is possible to do a simple comparison of proportions to see whether the post-intervention scores are reliably higher than pre-intervention scores for each item set. For 14 of the 16 cases, there is a statistically significant increase in scores from pre-intervention to post-intervention for target items (corresponding to those with lines that extend vertically above the dotted line), whereas this is the case for only two of the cases when control items are considered (corresponding to cases which show horizontal movement from pre-intervention to post-intervention). 
...
colnames(mytab)<-c('N_per_Set',paste0('.',100*mytab[1,2:9]))
mytab<-mytab[-1,]
...
ft<-flextable(mytab)
ft <- add_header_row(
  x = ft, values = c(" ", "Trained - untrained (proportion correct) difference"),
  colwidths = c(1, 6))
ft <- theme_box(ft)
...
### Single case trial designs for interventions with enduring effects
