...
The single case design, also known as N-of-1 trial, or small N design, is a commonly used intervention design in clinical psychology, speech and language therapy, and neuropsychology, including aphasia therapy [@perdices2009]. The single case design may be regarded as an extreme version of a _within-subjects_ design, where comparisons are made between conditions within a single person. Very often, a set of single case trials is combined into a *case series* (see below). It is important to note that a single case trial is not a simple case report, but rather a study that is designed and analysed in a way that controls as far as possible for the kind of unwanted influences on results described in chapters 2-5. 

Table \@ref(tab:scdlogic) shows the contrast in logic between the standard RCT and single case designs.
...
With single case trials, we do not have to worry about variation associated with individual participant characteristics - indeed we are interested in how different people respond to intervention -  but we do need to control as far as possible for other sources of variation. The ABA design involves contrasting an outcome during periods of intervention versus periods of no intervention. For example, @armson1998 studied the impact of an frequency-altered auditory feedback on 12 people who stuttered, in two condition, oral reading, and self-formulated speech. They contrasted a baseline period (A), a period with auditory feedback (B), and a post-intervention period (A). Figure \@ref(fig:SingleCaseDplot) shows data from two participants during the reading condition. Although the range of change differs for the two individuals, in both cases there is a precipitate drop in stuttering at the onset of the masking, which is sustained for some minutes before gradually rising back towards baseline levels. 

```{r SingleCaseDplot, echo=F, message=F, warning=F, fig.width=8,fig.height=4, fig.cap="Outcome over time in a single case ABA design. Digitised data from two participants in Figure 2 of Armson et al (2008)"}
...
While the characteristics in the second sentence fit well with speech-and-language therapy interventions, the first requirement - that the intervention should "act and cease to act quickly" is clearly inapplicable. As described in the previous chapter, with few exceptions, interventions offered by those working in education as well as speech and language therapists and those working in other allied health professions are intended to produce long-term change that persists long after the therapy has ended. Indeed, a therapy that worked only during the period of administration would not be regarded as a success. This means that some single case designs, notably ABA designs, which compare an outcomes for periods with (B) and without (A) intervention, anticipating that scores will go up transiently during the intervention block, will be inapplicable. In this regard, behavioural interventions are quite different from many pharmaceutical interventions, where ABA designs are increasingly being used to compare a series of active and washout periods for a drug. 

Despite this limitation, it is feasible to use an approach where we compare different time periods with and without intervention in some situations, most notably when there is good evidence that the targeted behaviour is unlikely to improve spontaneously. Inclusion of a baseline period, where behaviour is repeatedly sampled before intervention has begun, may give confidence that this is the case. An example of this __multiple baseline__ approach from a study by @swain2020 is discussed below. Where the same intervention can be applied to a group of participants, then a hybrid method known as the __multiple baseline across participants__ design can be used, which combines both between and within-subjects comparisons. A study by @koutsoftas is used to illustrate this approach below. 

One feature of many behavioural interventions (including those used in education and clinical psychology as well as in speech and language therapy) makes them excellent candidates for an alternative kind of single case trial design, the __multiple baseline across behaviours__ design. This is applicable where a single intervention has potential to target several specific behaviours or skills. This gives this field an edge that drug trials often lack: we can change the specific outcome that is targeted by the intervention and compare it with another outcome that acts as a within-person control measure. The example above of comparing trained vs untrained words in aphasia therapy provides an example of this approach [@best2013]. However, identifying alternative outcomes to compare is not always straightforward. We consider below a study by @leniston2021 that illustrates this approach.  


### Example of an N-of-1 case series  
The terms 'single case' and 'N-of-1' are misleading in implying that studies only involve one trained person. More commonly, studies assemble a series of N-of-1 cases. Where the same intervention is used for all cases, regular group statistics may be applied. But unlike in RCTs, heterogeneity of response is expected and documented. In fact, for that kind of study, the interest is less in whether an overall intervention effect is statistically significant, as in whether the data provide evidence of individual variation in response to intervention, as this is what would justify analysis of individual cases. Formally, it is possible to test whether treatment effects vary significantly across participants by comparing a model that does or does not contain a term representing this effect, using [linear mixed models] (https://stackoverflow.com/questions/24019807/how-to-compare-a-model-with-no-random-effects-to-a-model-with-a-random-effect-us), but we would recommend you consult a statistician, as those methods are complex and require specific types of data. In practice, it is usually possible to judge how heterogeneous responses to intervention are by inspecting plots of individual participants.  

Typically the small sample sizes in N-of-1 case series preclude any strong conclusions about the characteristics of those who do and do not show intervention effects, but results may subsequently be combined across groups, and specific hypotheses formulated about the characteristics of those who show a positive response. We consider an example of this approach by @best2013 below. 

### Example of multiple baseline design: Speech and language therapy for adolescents in youth justice.
This figure from @swain2020 provides a neat synopsis of the multiple baseline design approach. 

![Data from one case in the @swain2020 study](images/swain_eg.png)
...
plot(G1,type='b',main='Probe scores for staggered intervention groups',xlab='Measurement',ylab='Mean number correct /3',ylim=c(0,3),col="darkred",pch=G1p)
text(1.4,2.9,'Baseline',cex=.7,col="darkred")
text(5.8,2.8,'Intervention',cex=.7,col="darkred")
text(11,2.9,'Post',cex=.7,col="darkred")
arrows(0.5, 2.8, 2, 2.8,col='darkred',code=3,length=.1)
arrows(3, 2.7, 8, 2.7,col='darkred',code=3,length=.1)
arrows(9, 2.8, 12, 2.8,col='darkred',code=3,length=.1)
...
```
It is evident from inspection that in both groups a high proportion of children learned to do the phoneme awareness task they were trained on (which involved identifying the first sound in a word). The researchers reported effect sizes for individual children that were calculated by comparing score in the post-intervention period with those in the baseline period, and reported that children also showed improvements in curriculum based measures that had not specifically been trained.
One can see that there is quite a bit of variation between groups, and the researchers considered various factors that might explain this, but did not find any clear explanatory factors, either in terms of age, or language background.  
Despite the evident gains in scores seen in many children, the study did not provide convincing evidence that it had fully accounted for maturation or practice effects. The potential problem is that these young children were at an age when phonological awareness often starts to develop, and they were in classrooms where teaching was targeted on pre-literacy skills. The study included a manipulation that could have ruled out this explanation - different lengths of baseline period - but the data did not give clear evidence. We would have have greater confidence in the intervention effect if the scores of the two groups had started to diverge at measurements 3 and 4, which correspond to baseline in Group B and to intervention in Group A. 
The failure to see any such effect doesn't mean that intervention was ineffective: it just adds  ambiguity to the interpretation of the gains in scores. This could possibly have been overcome with a longer baseline period in group B, though the authors' account suggests this might have been difficult to accommodate in the school setting, where there was a limited time window to complete the study.  

### Example of multiple baseline across behaviours: Effectiveness of electropalatography
...
@best2013 evaluated rehabilitation for anomia in acquired aphasia. The researchers contrasted naming ability for words that had been trained, using a cueing approach, versus a set of untrained control words, a multiple baseline across behaviours design. In general, results were consistent with prior work in showing that improvement was largely confined to trained words. As noted above, this result allows us to draw a clear conclusion that the intervention was responsible for the improvement, but from a therapeutic perspective it was disappointing, as one might hope to see generalisation to novel words.  
...
Remedies <- c('Control outcome/time period','Control outcome/time period','Control outcome/time period','Baseline period to establish variability','Outcomes with low measurement error','NA','-','Record sessions','Strictly specified protocol','Report on who dropped out','A priori power analysis; need for many observations','Registration of trial protocol')
...
<!--The CENT reporting guidelines [67], [68] are intended for medical single case trials, the SCRIBE 2016 guidelines [22], [51] are intended for SCEDs in the behavioral sciences
