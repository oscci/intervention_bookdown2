...
```{r strattable,echo=F,message=F,warning=F, tab.caption = 'Randomized allocation by stratas: City, Age'}
...
```{r minitable,echo=F,message=F,warning=F,tab.caption = 'Randomized allocation by minimization'}
options(kableExtra.html.bsTable = T)
...
knitr::kable(mini.mat,escape = F, align = "c", booktabs = T) %>%
  kable_styling(c("striped", "bordered"), latex_options = "striped", full_width = F) %>%
  column_spec(1, bold = T)%>%
  group_rows("Sex",1,2)%>%
